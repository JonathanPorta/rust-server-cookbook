...
local function FindPlayer(self, player, target)
    ...
    while players:MoveNext() do
        if string.find(players.Current.displayName:lower(), target:lower(), 1, true) or rust.UserIDFromPlayer(players.Current) == target then
            table.insert(matches, players.Current)
        end
    end
